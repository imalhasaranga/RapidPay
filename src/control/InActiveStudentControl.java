        ...
        if (OccationalLogics.showOption("Yes", "No") == 0) {
            try {
                for (int i = 0; i < jt.getRowCount(); i++) {
                    ...
                    DB.stexecuteUpdate("update user_main set is_active = '" + (jt.getValueAt(i, 4).toString().equals("InActive") ? "0" : "1") + "' where User_id = '"+student_id+ "'");
                    
                    //Code to remove details from the fringer print table
                    DB.stexecuteUpdate("delete from Fingered_Students where Student_id = '"+student_id+"'");
                }
            } catch (Exception e) {
                System.out.println(e);
            }
